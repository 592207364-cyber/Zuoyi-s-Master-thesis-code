...
for phi=0:pi/4:pi/4
    ...
    for M=2:2:6
        ...
        for gamma0=0.2:0.2:0.8
            ...
            plot(kh(1:314-2*(phi==0)-gamma0*10*(phi==0)),cgcfdvs_c(1:314-2*(phi==0)-gamma0*10*(phi==0)),Color=color(colorindex,:),LineWidth=2,DisplayName=['Elastic CG-FD S-wave dispersion, γ=',num2str(gamma0)]);hold on;grid on;
        ...
        % sourcetype=1;% =1: Pressure;
         ...
         SG_A_Main_code_for_numerical_dispersion;
          plot(kh(1:314-2*(phi==0)-12*(phi==0)*(M==2)-2*(phi==pi/4)*(M==2)),sgcfdvs_c(1:314-2*(phi==0)-12*(phi==0)*(M==2)-2*(phi==pi/4)*(M==2)),'k',LineWidth=2,DisplayName='SG-FD S-wave dispersion',Visible='on');hold on;grid on;
        ...
        legend show;
        xlabel('kh');ylabel('c_f_d/c');
        title_str = sprintf('c_{fd}/c - kh, \\theta = %.0f^\\circ, 2M = %d, v_s = %.1f m/s', ...
                    phi/pi*180, M, vs0);
        title(title_str, 'Interpreter', 'tex', 'FontSize', 12, 'FontWeight', 'bold');
        set(gcf, 'Position', [0, 0, 600, 900], 'Color', 'w');
    end
    saveas(gcf,fullfile([ResultFig,'/Numerical_Dispersion'],['S_cfd_c_kh_theta_',num2str(phi/pi*180),'_2M_2_4_6.png']));
end