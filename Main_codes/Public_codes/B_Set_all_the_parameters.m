...
routputx=zeros(nr,nt);routputz=zeros(nr,nt);
    disp('Velocity model1: homogeneous model');
    disp('v_p=2000 m/s, v_s=800 m/s');
...
routputx=zeros(nr,nt);routputz=zeros(nr,nt);
    ...
    vp=zeros(nx,nz)+vp0;             %m/s P wave velocity
    vp(xs+squarex1:xs+squarex2,zs+squarez1:zs+squarez2)=vp1;
    vs=zeros(nx,nz)+vs0;               %m/s S wave velocity
    vs(xs+squarex1:xs+squarex2,zs+squarez1:zs+squarez2)=vs1;
    rho=zeros(nx,nz)+rho0;             %density kg/m³
    
    rho(xs+squarex1:xs+squarex2,zs+squarez1:zs+squarez2)=rho1;
    w=1./rho;
    alpha=zeros(nx,nz); alpha=alpha+vp;  %P wave velocity
    beta=zeros(nx,nz); beta=beta+vs;%S wave velocity field
end
...
switch FD
    case 'CG'
        ...
        rickerp=Z_Function_Ricker_wave(t,fc,1.0*dt); % pressure source
        % figure(1)
